...
class DiffusionModel(nn.Module):
    ...
    def sample(self, num_samples: int, size, device, noise: torch.Tensor = None) -> torch.Tensor:
        ...
        x_i = torch.randn(num_samples, *size).to(device) if noise is None else noise # fix не было привязки к девайсу

        for i in range(self.num_timesteps, 0, -1): 
            z = torch.randn(num_samples, *size, device=device) if i > 1 else 0  # fix не было привязки к девайсу
